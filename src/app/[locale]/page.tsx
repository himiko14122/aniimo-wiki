...
import { UNITS, TRAITS, MODES, EVOLUTIONS, TIER_COLOR_MAP } from '@/data/game-data';
...
export default async function HomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div>
      ...
      {/* ===== UNIT TABLE (Data-Driven Table 1) ===== */}
      <section className="bg-[var(--color-bg-secondary)] border-y border-[var(--color-border)] full-width-bg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="flex items-center justify-between mb-8">
            <div>
              <div className="section-label">{t('home_module_creatures')}</div>
              <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_unit_table_title')}</h2>
            </div>
            <Link href="/creatures" title={t('home_module_creatures')} className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
          </div>
          <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_unit_table_desc')}</p>
          <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] overflow-hidden backdrop-blur-sm">
            <div className="grid grid-cols-[2rem_1fr_4rem_1.5fr_1.5fr] sm:grid-cols-[2.5rem_1.5fr_4rem_2fr_2fr] gap-x-4 text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider border-b border-[var(--color-border)] px-4 py-3">
              <span></span><span>{t('home_unit_name')}</span><span>{t('tierList_tierLabel')}</span><span>{t('home_unit_role')}</span><span>{t('home_unit_element')}</span>
            </div>
            {UNITS.map((unit, idx) => {
              const tc = tierColorVal(unit.tier);
              return (
                <Link key={unit.id} href="/creatures" title={t(unit.nameKey)} className="grid grid-cols-[2rem_1fr_4rem_1.5fr_1.5fr] sm:grid-cols-[2.5rem_1.5fr_4rem_2fr_2fr] items-center gap-x-4 px-4 py-3 border-b border-white/[0.04] last:border-b-0 hover:bg-[var(--color-accent)]/5 transition-colors group">
                  ...
                  <span className="justify-self-start text-[10px] font-bold px-1.5 py-0.5 rounded-full" style={{ color: tc, background: `${tc}15` }}>{unit.tier}</span>
                  <span className="text-xs text-[var(--color-text-secondary)]">{t(unit.roleKey)}</span>
                  <span className="text-xs text-[var(--color-text-secondary)]">{t(unit.elementKey)}</span>
                </Link>
              );
            })}
          </div>
      ...
      {/* ===== TRAIT CARDS (Data-Driven Cards 1) ===== */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex items-center justify-between mb-8">
          <div>
            <div className="section-label">{t('home_module_twine')}</div>
            <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_trait_cards_title')}</h2>
          </div>
          <Link href="/twine" title={t('home_module_twine')} className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
        </div>
        <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_trait_cards_desc')}</p>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {TRAITS.map((trait) => {
            const tc = trait.tier === 'S' ? 'var(--color-tier-s)' : trait.tier === 'A' ? 'var(--color-tier-a)' : 'var(--color-tier-b)';
            return (
              <Link key={trait.id} href="/twine" title={t(trait.nameKey)} className="category-card group block rounded-2xl overflow-hidden">
                <div className="p-5">
                  <div className="flex items-center gap-2 mb-3">
                    <Sparkles className="w-4 h-4 text-[var(--color-accent)]" />
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ color: tc, background: `${tc}15` }}>{trait.rarityKey && t.has(trait.rarityKey) ? t(trait.rarityKey) : trait.tier}</span>
                  </div>
                  <h3 className="text-[0.9375rem] font-bold font-[var(--font-heading)] text-[var(--color-text-primary)] group-hover:text-[var(--color-accent)] transition-colors mb-1">{t(trait.nameKey)}</h3>
                  <p className="card-meta text-[0.75rem] text-[var(--color-text-secondary)] mb-2"><span className="meta-label">{t('home_trait_effect')}: </span><span className="meta-value">{t(trait.effectKey)}</span></p>
                  <p className="card-desc text-[0.8125rem] text-[var(--color-text-secondary)]">{t('home_trait_best')}: {t(trait.bestKey)}</p>
                </div>
              </Link>
            );
          })}
      ...
      {/* ===== GAME MODE TABLE (Data-Driven Table 2) ===== */}
      <section className="bg-[var(--color-bg-secondary)] border-y border-[var(--color-border)] full-width-bg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="flex items-center justify-between mb-8">
            <div>
              <div className="section-label">{t('home_module_multiplayer')}</div>
              <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_mode_table_title')}</h2>
            </div>
            <Link href="/multiplayer" title={t('home_module_multiplayer')} className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
          </div>
          <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_mode_table_desc')}</p>
          <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] overflow-hidden backdrop-blur-sm">
            <div className="grid grid-cols-[2rem_1fr_4rem_1.5fr_2fr] sm:grid-cols-[2.5rem_1.5fr_4rem_2fr_2.5fr] gap-x-4 text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider border-b border-[var(--color-border)] px-4 py-3">
              <span></span><span>{t('home_mode_mode')}</span><span>{t('tierList_tierLabel')}</span><span>{t('home_mode_focus')}</span><span>{t('home_mode_best')}</span>
            </div>
            {MODES.map((mode, idx) => {
              const tc = tierColorVal(mode.tier);
              return (
                <Link key={mode.id} href="/multiplayer" title={t(mode.nameKey)} className="grid grid-cols-[2rem_1fr_4rem_1.5fr_2fr] sm:grid-cols-[2.5rem_1.5fr_4rem_2fr_2.5fr] items-center gap-x-4 px-4 py-3 border-b border-white/[0.04] last:border-b-0 hover:bg-[var(--color-accent)]/5 transition-colors group">
                  <span className="text-xs font-mono text-[var(--color-text-muted)]">{idx + 1}</span>
                  <span className="flex items-center gap-2 font-semibold text-sm group-hover:text-[var(--color-accent)] transition-colors">
                    <Gamepad2 className="w-4 h-4 text-[var(--color-accent)]" />
                    {t(mode.nameKey)}
                  </span>
                  <span className="justify-self-start text-[10px] font-bold px-1.5 py-0.5 rounded-full" style={{ color: tc, background: `${tc}15` }}>{mode.tier}</span>
                  <span className="text-xs text-[var(--color-text-secondary)]">{t(mode.focusKey)}</span>
                  <span className="text-xs text-[var(--color-text-secondary)]">{t(mode.bestKey)}</span>
                </Link>
              );
            })}
          </div>
      ...
      {/* ===== EVOLUTION CARDS (Data-Driven Cards 2) ===== */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex items-center justify-between mb-8">
          <div>
            <div className="section-label">{t('home_module_evolution')}</div>
            <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_evo_cards_title')}</h2>
          </div>
          <Link href="/evolution" title={t('home_module_evolution')} className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
        </div>
        <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_evo_cards_desc')}</p>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {EVOLUTIONS.map((evo) => {
            const tc = evo.tier === 'S' ? 'var(--color-tier-s)' : evo.tier === 'A' ? 'var(--color-tier-a)' : evo.tier === 'B' ? 'var(--color-tier-b)' : 'var(--color-tier-c)';
            return (
              <Link key={evo.id} href="/evolution" title={t(evo.nameKey)} className="category-card group block rounded-2xl">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ color: tc, background: `${tc}15` }}>{t('tierList_tierLabel')} {evo.tier}</span>
                  <TrendingUp className="w-4 h-4 text-[var(--color-accent)]" />
                </div>
                <h3 className="text-[0.9375rem] font-bold font-[var(--font-heading)] text-[var(--color-text-primary)] group-hover:text-[var(--color-accent)] transition-colors mb-1">{t(evo.nameKey)}</h3>
                <p className="card-meta text-[0.75rem] text-[var(--color-text-secondary)] mb-2"><span className="meta-label">{t('home_evo_source')}: </span><span className="meta-value">{t(evo.sourceKey)}</span></p>
                <p className="card-desc text-[0.8125rem] text-[var(--color-text-secondary)]">{t('home_evo_use')}: {t(evo.useKey)}</p>
              </Link>
            );
          })}
        </div>
